...
@pytest.mark.asyncio
async def test_product_lifecycle(client: AsyncClient, auth_headers: dict):
    ...
    search_response = await client.get(
        "/products/search?q=Integration Test",
        headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_product_price_history(client: AsyncClient, auth_headers: dict):
...
@pytest.mark.asyncio
async def test_product_validation(client: AsyncClient, auth_headers: dict):
...
@pytest.mark.asyncio
async def test_duplicate_product_handling(client: AsyncClient, auth_headers: dict):
    ...
    assert second_response.status_code == 400  # Should prevent duplicates
