...
async def async_process_pending_notifications():
    ...
    if not bot:
        logger.error("Telegram bot is not initialized - missing TELEGRAM_BOT_TOKEN")
        return {"error": "Telegram bot is not initialized"}
...
async def send_price_drop_notification(
    ...
    try:
        ...
        if product.affiliate_link:
            ...
            # Validate URL for Telegram
            safe_url = product.url
            if "demo-server" in product.url or "localhost" in product.url:
                # Telegram rejects local URLs. Use a fallback for dev.
                safe_url = "https://example.com/product-placeholder"
            keyboard_buttons.append([
                InlineKeyboardButton(text="🔗 View Product", url=safe_url)
            ])
